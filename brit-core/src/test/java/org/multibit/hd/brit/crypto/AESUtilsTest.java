...
public class AESUtilsTest {
  ...
  public void setUp() throws Exception {
    ...
    initialisationVector = new byte[org.multibit.hd.brit.core.crypto.AESUtils.BLOCK_LENGTH];
    secureRandom.nextBytes(initialisationVector);

    // Create a random key
    secureRandom.nextBytes(initialisationVector);
    byte[] keyBytes = new byte[org.multibit.hd.brit.core.crypto.AESUtils.KEY_LENGTH];
  ...
  public void testEncrypt_ExpectSuccess1() throws Exception {
    ...
    // Encrypt
    byte[] encryptedBytes = org.multibit.hd.brit.core.crypto.AESUtils.encrypt(plainBytes, keyParameter, initialisationVector);
    log.debug("Encrypted message: '{}'", Utils.HEX.encode(encryptedBytes));

    // Decrypt
    byte[] rebornBytes = org.multibit.hd.brit.core.crypto.AESUtils.decrypt(encryptedBytes, keyParameter, initialisationVector);
